Office.onReady(() => {
    document.getElementById("compareBtn").addEventListener("click", async function () {
        ...
        try {
            ...
            alert("An error occurred while reading the Excel files.");
        }
    });
});
    ...
    return new Promise((resolve, reject) => {
        ...
        reader.onload = function (e) {
            try {
                ...
                const cleaned = json
                    .filter(row => row.length >= 2) // must have at least ID + Description
                    .slice(1) // skip first row in case it's a header
                    .map(row => ({
                        id: row[0]?.toString() ?? "",
